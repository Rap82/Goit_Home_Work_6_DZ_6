...
def get_cats_info ( path ) : # Функцію add_employee_to_file(record, path), яка у файл (параметр path - шлях до файлу) 
                                            # буде додавати співробітника (параметр record) у вигляді рядка "Drake Mikelsson,19".
    ...
    with open (path, 'r') as fh :     # Відкриття файлу для читання через Менеджер контексту *with open('text.txt', 'w+') as fh: #Більше про менеджер дивись на початку завдання
        ...
        for cat_info in cats_info :  # Цикл в якому будемо проходити по кожному елементу з списку *cats_info і замінювати символ переносу на новий рядок *\n , на пустий символ "" .
            ...
            cat_info_dict_up = dict( zip ( Key_for_dict, cat_info_list ) ) # *cat_info_dict_up отримає занчення типу словник сформований за допомогою вбудованих функції *dict () і *zip (*list , *list)
                                                                         ...
                                                                         #   *zip(Key_for_dict,cat_info_list) функція отримує два значення , список *Key_for_dict і *cat_info_lis і повертає список де кожен елемент списку буде сформований в форматі
